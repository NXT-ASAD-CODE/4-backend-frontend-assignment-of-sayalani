              ...
              {products.map((product) => {
                ...
                return (
                  <tr key={getProductKey(product)} style={{ borderBottom: '1px solid #f0f2f5' }}>
                    <td style={{ padding: '14px 12px', color: '#111936', fontWeight: 700 }}>{isEditing ? <div style={{ display: 'grid', gap: '8px' }}><input name="title" value={editForm.title} onChange={updateEditField} style={{ padding: '9px', color: '#fff', background: '#111936', border: '1px solid #344054', borderRadius: '7px' }} /><input name="image" type="url" value={editForm.image} onChange={updateEditField} placeholder="Image URL" aria-label="Product image URL" style={{ padding: '9px', color: '#fff', background: '#111936', border: '1px solid #344054', borderRadius: '7px' }} /></div> : (product.title || product.name)}</td>
